...
class RedirectIfLink
{
    ...
    private function findLinkGroup($hash): ?LinkGroup
    {
        ...
        if ($groupData) {
            $model =
                $groupData->type === 'biolink' ? Biolink::class : LinkGroup::class;
            $model::unguard();
     ...
     */
    private function handleLinkeable($linkeable, Request $request)
    {
        if (app(LinkeablePublicPolicy::class)->isAccessible($linkeable)) {
            // redirect to stats page
            if (Str::endsWith($request->getUri(), '+')) {
                $route = Str::plural(Str::snake(class_basename($linkeable)));
            ...
            if ($linkeable instanceof Biolink) {
                $linkeable = app(LoadBiolinkContent::class)->execute(
                    $linkeable,
                    ['with' => 'appearance']
                );
            ...
            $response['seo'] = isset($response['seo']) ? $response['seo']->toArray() : [];
    ...
    /**
     * @param Link|LinkGroup|Biolink $linkeable
     * @return Link|LinkGroup|Biolink
     */
    private function applyRules($linkeable)
    {
