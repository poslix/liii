...
abstract class WorkspacedResourcePolicy extends BasePolicy
{
    ...
    protected function userHasPermission(User $user, string $permission): Response
    {
        ...
        $activeWorkspace = $this->getWorkspace();
        $userOwnsWorkspace  =
            !$activeWorkspace ||
        ...
        if ($userOwnsWorkspace && parent::userHasPermission($user, $permission)->denied()) {
        ...
        else if ( ! $userOwnsWorkspace) {
            $workspaceUser = $activeWorkspace->member($user->id);
            if ( ! $workspaceUser->hasPermission($permission)) {
                return Response::deny('No permission', self::NO_WORKSPACE_PERMISSION);
            }
        }
    ...
    private function userIsWorkspaceMember(User $user): bool
    {
        if ($workspace = $this->getWorkspace()) {
            return !$workspace->personal() && $workspace->member($user->id);
        } else {
            return false;
        }
    }

    protected function getWorkspace(): ?ActiveWorkspace
    {
        // request is from admin and not for user dashboard
        if (!$this->request->header(ActiveWorkspace::HEADER)) {
            return null;
        }
        return app(ActiveWorkspace::class);
    }
}
